...
import { deleteShoutOut } from "@/lib/actions/shout-out.actions";
...
const DeleteShoutOutButton = ({
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const res = await deleteShoutOut(shoutOutId);
      if (res.success) {
        toast.success("Shout Out deleted successfully!");
        if (onDelete) {
          await onDelete();
        } else {
          router.refresh();
      ...
      toast.error("An error occurred while deleting the shout out.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            This action cannot be undone. This will permanently delete the Shout
            Out from the server and off of your site.
          </DialogDescription>
          <div className="mt-4">
            <label
...
export default DeleteShoutOutButton;
